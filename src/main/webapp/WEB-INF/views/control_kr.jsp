...
<title>Control ${market} — AUTO TRADING</title>
...
.sym-opt-mkt{
  font-family:var(--mono);font-size:8px;color:var(--lime);
  border:1px solid var(--lime-b);background:var(--lime-d);border-radius:12px;padding:2px 7px;
}
      ...
      <div class="card-bd">
        <div class="input-row">
          <div class="suggest-wrap">
            <input class="tb-input" id="symInput" placeholder="종목코드/이름 입력 (예: 005930, 삼성전자)" maxlength="24" autocomplete="off" spellcheck="false"/>
            <div class="sym-suggest" id="symSuggest"></div>
          </div>
          <input class="tb-input amount-input" id="buyAmountInput" type="number" min="0" step="1" placeholder="종목당 금액"/>
  ...
  <div class="col">
    <div class="card" style="flex:1;min-height:0;animation:fu .3s .05s ease both;">
      <div class="card-hd">
        ...
        <span class="badge" id="ordSource" style="font-size:7px;">/api/orders/${market == 'US' ? 'us' : 'kr'}</span>
      </div>
...
const MARKET = '${market != null ? market : "KR"}';
  ...
  symSuggestEl.innerHTML = suggestRows.map((row, idx) =>
    '<div class="sym-opt'+(idx===suggestIdx?' act':'')+'" data-idx="'+idx+'">'
      +'<div class="sym-opt-l">'
        ...
        +'<div class="sym-opt-name">'+esc(row.name||row.symbol)+'</div>'
      +'</div>'
...
function fetchSuggestRows(raw){
  ...
  const url = B
    + '/api/market/symbol-suggest?market=' + encodeURIComponent('KR')
    + '&exch=' + encodeURIComponent('KRX')
    ...
    .then(data => {
      ...
      const mapped = rows
        .map(row => ({
          ...
          market: String(row.market || 'KR').toUpperCase()
        }))
...
window.startTop=function(){
  ...
  if(topSel.size>0){
    ...
    const sels=raw.filter(s=>/^[0-9]{6}$/.test(String(s||'')));
    if(!sels.length){
