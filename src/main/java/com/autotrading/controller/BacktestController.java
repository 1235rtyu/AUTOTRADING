...
public class BacktestController {
    ...
    private String buildAiPrompt(Map<String, Object> params, List<Map<String, Object>> results) {
        ...
        sb.append("아래는 1분봉 기반 자동매매 전략(BREAKOUT / PULLBACK / STRONG_PULLBACK / VWAP_RECLAIM)의\n");
        ...
        sb.append(pLine("최소 현재가",            params, "minPrice",            "500원"));
        ...
        sb.append(pLine("Market Filter",          params, "useMarketFilter",     "false"));
        sb.append(pLine("매수 쿨다운",            params, "buyCooldownSec",      "0초  (같은 종목 재진입 대기)"));
        sb.append(pLine("일일 최대 진입 수",       params, "maxDailyEntryCount",  "2건"));
        sb.append(pLine("같은 패턴 최대 진입",     params, "maxSamePatternEntry", "1건"));

        // 진입 등급 필터
        sb.append("\n▶ 진입 등급 필터\n").append(SEP2).append("\n");
        sb.append(pLine("S등급(90~94) 차단",      params, "blockSGrade", "true  (실증 분석에서 승률 최저 구간)"));
        sb.append(pLine("A등급(85~89) 차단",      params, "blockAGrade", "false"));

        // PULLBACK 진입
        sb.append("\n▶ PULLBACK 진입 조건\n").append(SEP2).append("\n");
        sb.append(pLine("활성 여부",              params, "enablePullback",                  "true"));
        ...
        sb.append(pLine("최소 점수(0~100)",        params, "breakoutMinScore",             "78"));
        sb.append(pLine("VWAP 최대 이격",          params, "vwapMaxGapBreakoutPct",        "2.2%"));
        ...
        sb.append(pLine("고정 익절",               params, "breakoutTpPct",     "2.8%"));
        sb.append(pLine("트레일 시작 수익",         params, "breakoutTrailSt",   "2.3%"));
        ...
        sb.append(pLine("최근 고점 대비 눌림 최소",params, "spPullbackMinPct",    "1.2%"));
        sb.append(pLine("최근 고점 대비 눌림 최대",params, "spPullbackMaxPct",    "2.5%"));
        sb.append(pLine("VWAP 위 최소 위치",       params, "spVwapMinAbovePct",   "0.2%  (현재가가 VWAP 대비 최소 이 % 위)"));
        sb.append(pLine("3봉 vs 10봉 거래량 비율", params, "spVol3RatioMax",      "0.6  (3봉 평균 < 10봉 평균 × 이 값 → 거래량 감소 확인)"));
        sb.append(pLine("양봉 몸통 비율 최소",     params, "spBodyRatioMin",      "0.6  (몸통/전체 캔들 크기)"));
        sb.append(pLine("최소 점수",              params, "spMinScore",           "95"));
        ...
        sb.append(pLine("VWAP 이탈 확인 봉수",    params, "vrLookbackBars",     "5봉  (최근 N봉 내 VWAP 아래 봉 존재 확인)"));
        sb.append(pLine("거래량 배수",             params, "vrVolMult",          "2.5x  (5봉 평균 대비)"));
        sb.append(pLine("연속 VWAP 위 봉 수",     params, "vrMinAboveVwapBars", "5봉  (회복 후 연속 N봉 이상 VWAP 위)"));
        sb.append(pLine("최소 점수",              params, "vrMinScore",          "90"));

        // VWAP_RECLAIM 청산
        sb.append("\n▶ VWAP_RECLAIM 청산 조건\n").append(SEP2).append("\n");
        sb.append(pLine("손절",                    params, "vrStopPct",   "1.5%"));
        sb.append(pLine("고정 익절",               params, "vrTpPct",     "2.0%"));
        sb.append(pLine("트레일 시작 수익",         params, "vrTrailSt",   "1.5%"));
        sb.append(pLine("트레일 고점 대비 하락",    params, "vrTrailDrop", "1.0%"));

        // 공통 청산 조건
        sb.append("\n▶ 공통 청산 조건\n").append(SEP2).append("\n");
        sb.append(pLine("긴급 손절",              params, "emergencyStopPct",  "5.5%  (모든 조건 무시, 즉시 청산)"));
        ...
        sb.append(pLine("VWAP Break 유예시간",    params, "vwapBreakGraceSec", "360초  (진입 후 N초 이내 VWAP Break 무시)"));
        ...
        sb.append(pLine("Breakeven 손실 한도",    params, "breakevenLoss",     "-0.3%  (Guard 활성 후 이 손실 시 청산)"));
        ...
        sb.append(pLine("EOD 강제 청산",          params, "useEodForceSell",   "true  (14:49 이후 포지션 강제 청산)"));
        ...
        sb.append("   - BREAKOUT / PULLBACK / STRONG_PULLBACK / VWAP_RECLAIM 각 모드의 강약점\n");
        sb.append("   - 비활성화하거나 조건을 강화해야 할 모드 추천\n\n");
        sb.append("3. 진입 조건 최적화\n");
        sb.append("   - 현재 최소 점수(BREAKOUT=" + str(params, "breakoutMinScore", "78")
                + ", PULLBACK=" + str(params, "pullbackMinScore", "80")
                + ", SP=" + str(params, "spMinScore", "95")
                + ", VR=" + str(params, "vrMinScore", "90") + ")가 적절한지 검토\n");
        ...
        sb.append("   - VWAP 이격 기준(PB=" + str(params, "vwapMaxGapPullbackPct", "1.0")
                + "%, BO=" + str(params, "vwapMaxGapBreakoutPct", "2.2") + "%)이 적절한지\n");
        ...
        sb.append("   - BREAKOUT: 손절=" + str(params, "breakoutStopPct", "2.0")
                + "%, 익절=" + str(params, "breakoutTpPct", "2.8")
                + "%, 트레일시작=" + str(params, "breakoutTrailSt", "2.3")
        ...
        sb.append("   - VR: 손절=" + str(params, "vrStopPct", "1.5")
                + "%, 익절=" + str(params, "vrTpPct", "2.0")
                + "%, 트레일시작=" + str(params, "vrTrailSt", "1.5")
                + "%, 트레일하락=" + str(params, "vrTrailDrop", "1.0") + "%\n");
        sb.append("   - 긴급손절=" + str(params, "emergencyStopPct", "5.5")
                + "%, VWAP Break 버퍼=" + str(params, "vwapBreakBuffer", "0.5")
                + "%, 유예=" + str(params, "vwapBreakGraceSec", "360") + "초 검토\n\n");
