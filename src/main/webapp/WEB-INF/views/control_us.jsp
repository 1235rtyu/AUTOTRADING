...
.topbar::after{content:'';position:absolute;bottom:0;left:0;right:0;height:1px;pointer-events:none;
  background:linear-gradient(90deg,transparent,rgba(255,255,255,.05) 15%,rgba(91,163,255,.6) 50%,rgba(255,255,255,.05) 85%,transparent);}
.tb-logo{display:flex;align-items:center;gap:8px;padding:0 14px;height:100%;border-right:1px solid var(--rim);}
.logo-mk{width:26px;height:26px;background:linear-gradient(135deg,#7bbfff,#4090e0);border-radius:6px;
  display:flex;align-items:center;justify-content:center;box-shadow:0 0 12px rgba(91,163,255,.35);}
...
/* ══════════════════════════════
   PAGE LAYOUT
   [PNL 3칸 — 전체 너비]
   [left 380px | center 420px | right 1fr]
══════════════════════════════ */
.page{
...
.sym-opt-mkt{font-size:8px;color:var(--blue);border:1px solid var(--blue-b);background:var(--blue-d);border-radius:12px;padding:1px 6px;}
...
/* ══════════════════════════════
   CENTER COL — Volume Top 50 세로 리스트
══════════════════════════════ */
...
<nav class="topbar">
  ...
  <div class="tb-nav">
    ...
    <a class="tb-a cur" href="${pageContext.request.contextPath}/control/us">Control·US</a>
  ...
  <div class="tb-mkt"><span class="mkt-pill us">US</span></div>
  <div class="tb-clock">
...
<div class="page" id="page">
  ...
  <div class="pnl-row">
    ...
    <div class="pnl-box" style="border:1px solid rgba(91,163,255,.2);background:linear-gradient(148deg,rgba(20,25,42,.97),rgba(14,18,32,.95));">
      <div><div class="pnl-label" style="color:var(--blue);">오늘</div><div class="pnl-date" id="pnlDate2">--</div></div>
      <div style="text-align:right;margin-left:auto;">
        ...
        <div class="card-bd" style="flex:1;">
          <div class="input-row">
            <div class="suggest-wrap">
              <input class="tb-input" id="symInput" placeholder="종목코드 또는 이름 (예: NVDA)" maxlength="24" autocomplete="off" spellcheck="false"/>
              <div class="sym-suggest" id="symSuggest"></div>
            </div>
          </div>
          <div style="margin-top:4px;">
            <input class="tb-input" id="buyAmountInput" type="number" min="0" step="0.01" placeholder="종목당 금액" style="width:100%;"/>
          </div>
      ...
      <div class="card ticker-card">
        ...
        <div class="ticker-controls">
          <div class="ticker-ctrl-row">
            ...
            <input class="tb-input" id="buyAmountTop" type="number" min="0" step="0.01" placeholder="종목당 금액" style="flex:1;height:26px;font-size:10px;"/>
          </div>
          <div class="ticker-ctrl-row">
    ...
    <div class="right-col">
      <div class="card ord-section">
        <div class="card-hd">
          <div class="card-hd-l">
            <div class="hd-dot" style="background:var(--gold)"></div>
            <span class="card-title">Order History — US</span>
          </div>
          <span class="badge" style="font-size:7px;">/api/orders/us</span>
        </div>
          ...
          <table class="ord-table" id="ordTable">
            <thead>
              <tr>
                ...
                <th>사유</th>
                <th class="r" style="width:90px;">가격</th>
...
window.loadWatchlistToManual=function(){
  return get('/api/watchlist')
    .then(items=>{
      ...
      const syms=rows
        .filter(it=>{const ex=(it.exchange||'').toUpperCase();return ['NAS','NYS','AMS','US','NASD','NYSE','AMEX'].includes(ex)||/^[A-Za-z]/.test(it.symbol||'');})
...
function queueSuggest(raw){
  ...
  suggestTimer=setTimeout(()=>{
    ...
    const url=B+'/api/market/symbol-suggest?market=US&exch='+encodeURIComponent(EXCH)+'&limit='+SUGGEST_LIMIT+'&q='+encodeURIComponent(query);
    fetch(url,{cache:'no-store',signal:suggestAbortController.signal})
      .then(r=>r.ok?r.json():Promise.reject())
      .then(data=>{
        if(reqId!==suggestReqId)return;
        const rows=(Array.isArray(data&&data.data)?data.data:[])
          .map(row=>({symbol:String(row.symbol||'').toUpperCase(),name:String(row.name||'').trim(),market:'US'}))
          .filter(r=>r.symbol);
...
window.startManual=function(){
  ...
  Promise.all(manualSyms.map(sym=>{
    const body={symbol:sym,exchange:EXCH};
...
function renderPnl(pts){
  pts.forEach((p,i)=>{
    ...
    if(amtEl){amtEl.textContent=pnl===0?'±$0':fmtUsd(pnl);amtEl.className='pnl-amount '+(pnl>0?'pos':pnl<0?'neg':'zero');}
    if(dateEl)dateEl.textContent=fmtDate(p.date);
    if(subEl)subEl.textContent=pnl===0?'거래 없음':(pnl>0?'수익':'손실');
  });
}
function fetchPnl(){
  get('/api/pnl/recent?market=US&days=3')
    .then(pts=>{if(Array.isArray(pts)&&pts.length===3)renderPnl(pts);})
    .catch(()=>{});
}

/* ── Volume Top 50 — 세로 리스트 ── */
function renderTop(){
...
window.fetchTop=function(){
  return get('/api/market/ranking?market=US&exch='+encodeURIComponent(EXCH))
...
window.startTop=function(){
  ...
  if(buyAmount===undefined)return;
  if(topSel.size>0){
    const sels=Array.from(topSel).filter(s=>/^[A-Z][A-Z0-9.\-]{0,9}$/.test(String(s||'')));
  ...
  let url='/api/control/start-top?n='+n+'&minRate=0&market=US&exch='+encodeURIComponent(EXCH);
